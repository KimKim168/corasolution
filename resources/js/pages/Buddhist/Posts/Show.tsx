...
export default function Show() {
    const { showData, postImages, postFiles, relatedItems, relatedItemsTwo, app_url } = usePage<any>().props;
                    ...
                    <div className="mx-auto flex max-w-(--breakpoint-xl) flex-col items-start gap-8 px-4 lg:flex-row">
                        <div className="flex-1 space-y-4">
                            <div className="rounded-md bg-background p-6">
                                ...
                                <div className="rounded-md bg-background pt-6">
                                    <HtmlContentDisplay
                                        className="prose-img:rounded-md"
                                        content={
                                            currentLocale === 'kh'
                                ...
                                {postImages?.length > 0 && (
                                    <PostmagesGallery label={t('Images')} images={postImages} basePath="/images/thumb/" />
                                )}
                                {postFiles?.length > 0 && <UploadedFile label={t('Files')} files={postFiles} basePath="/assets/files/posts/" />}
                            </div>
